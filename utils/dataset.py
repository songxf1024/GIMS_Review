...
class COCO_loader(Dataset):
    def __init__(self, dataset_params, typ="train", limit=-1, color=False):
        ...
        self.images = self.coco_json.getImgIds()[:limit]
        self.color = color
        if self.apply_aug:
    ...
    def __getitem__(self, index: int):
        ...
        if resize:
            orig_resized = cv2.resize(image, (self.config['image_width'], self.config['image_height']), interpolation=cv2.INTER_AREA)
            warped_resized = cv2.resize(warped_image, (self.config['image_width'], self.config['image_height']), interpolation=cv2.INTER_AREA)
        else:
            orig_resized = image
...
class COCO_valloader(Dataset):
    def __init__(self, dataset_params, color=False):
    ...
    def __getitem__(self, index: int):
        ...
        image = cv2.imread(os.path.join(self.images_path, image_name), cv2.IMREAD_COLOR if self.color else cv2.IMREAD_GRAYSCALE)
        height, width = image.shape[0:2]
        warped_image = cv2.warpPerspective(image.copy(), homo_matrix, (width, height))
        orig_resized = image
        warped_resized = warped_image
...
def collate_batch(batch):
    ...
    orig_resized = np.stack([torch.from_numpy(i) for i in list_elem[0]], 0)
    warped_resized = np.stack([torch.from_numpy(i) for i in list_elem[1]], 0)
    homographies = torch.stack([torch.from_numpy(i) for i in list_elem[2]], 0)
    orig_warped_resized = np.concatenate([orig_resized, warped_resized], 0)
