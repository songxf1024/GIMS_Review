...
def read_UBC_patch_opencv(train_root, sz_patch, color=True):
    patch = []
    ...
    for i, img_file in enumerate(file):
        if img_file.find('bmp') > -1:
            if color:
                img = cv2.imread(os.path.join(train_root, img_file), cv2.IMREAD_COLOR)
                #img = cv2.cvtColor(img, cv2.COLOR_GRAY2BGR) #####
            else:
                img = cv2.imread(os.path.join(train_root, img_file), cv2.IMREAD_GRAYSCALE)
            #img_height, img_width = img.shape
            if color:
                img_height, img_width, img_channel = img.shape #height is row, width is column  ########
            else:
            ...
            for v in range(0, img_height, sz_patch_raw):  # Vertival
                for h in range(0, img_width, sz_patch_raw):  # Horizontal
                    if color:
                        patch_temp = img[v:v+sz_patch_raw, h:h+sz_patch_raw, :]  ########
                    else:
    ...
    if color:
        return np.array(patch).transpose(0,3,1,2)########
    else:
...
def read_hpatches_patch_opencv(train_root, sz_patch, color=True):
    sz_patch_raw = 65
    if sz_patch_raw != sz_patch:
        flag_resize = True
    patch = []
    ...
    for i, scene in enumerate(scene_file):#
        ...
        img_all = []
        if color:
            img_all.append(cv2.imread(os.path.join(train_root, scene, 'ref.png'), cv2.IMREAD_COLOR))  ##########
        else:
            img_all.append(cv2.imread(os.path.join(train_root, scene, 'ref.png'), cv2.IMREAD_GRAYSCALE))
        for j, set in enumerate(sets):
            for k in range(1, 6):
                if color:
                    img_all.append(cv2.imread(os.path.join(train_root, scene, set + str(k) + '.png'),
                                              cv2.IMREAD_COLOR))  ##########
                else:
                    img_all.append(cv2.imread(os.path.join(train_root, scene, set + str(k)+'.png'), cv2.IMREAD_GRAYSCALE))

        if color:
            img_height, _, _ = img_all[0].shape  ########
        else:
...
def load_UBC_for_train(data_root, train_set, sz_patch=32, nb_pt_each_batch=512, nb_pat_per_pt=2, epoch_max=200, flag_load_index=True, color=True): # all outputs are numpy arrays
    train_root = os.path.join(data_root, train_set)
    if color:
        file_data_train = os.path.join(train_root, train_set + '_sz' + str(sz_patch) + '_color.npz')
        file_index_train = os.path.join(train_root, train_set + '_index_train_ID' + str(nb_pt_each_batch) + '_pat' + str(nb_pat_per_pt) + '_color..npy')
    else:
...
def load_UBC_for_test(data_root, test_set, sz_patch=32, color=True): # all outputs are numpy arrays
    test_root = os.path.join(data_root, test_set)
    file_data_test = os.path.join(test_root, test_set + '_sz' + str(sz_patch) + '_color' + ('3' if color else '1') + '_100k_test.npz')
    ...
    if os.path.exists(file_data_test):
        ...
        file_data_train = os.path.join(test_root, test_set + '_sz' + str(sz_patch) + '_color' + ('3' if color else '1') + '.npz')
        if os.path.exists(file_data_train):
            ...
            print(test_set)
            patch_train = read_UBC_patch_opencv(test_root, sz_patch, color=color)
...
def load_hpatches_for_train(data_root, sz_patch, nb_pt_each_batch, nb_pat_each_pt, epoch_max, flag_load_index=True, color=True):
    train_set = 'hpatches'
    train_root = os.path.join(data_root, 'hpatches/data/hpatches-release')
    save_root = os.path.join(data_root, 'hpatches/data/')

    if color:
        file_data_train = os.path.join(save_root, train_set + '_sz' + str(sz_patch) + '_color.npz')
        file_index_train = os.path.join(save_root, train_set + '_index_train_ID' + str(nb_pt_each_batch) + '_pat' + str(nb_pat_each_pt) + '_color.npy')
    else:
    ...
    if os.path.exists(file_data_train):
        ...
        print(train_set)
        patch = read_hpatches_patch_opencv(train_root, sz_patch, color=color)
...
def load_hpatches_split_train(data_root, sz_patch, nb_pt_each_batch=512, nb_pat_each_pt=2, epoch_max=100, split_name='a', flag_load_index=True, flag_std_filter=True, color=True):
    train_set = 'hpatches'
    train_root = os.path.join(data_root, 'hpatches/data/hpatches-release')
    save_root = os.path.join(data_root, 'hpatches/data/')
    if color:
        file_data_train = os.path.join(save_root, train_set + '_sz' + str(sz_patch) + '_split_' + split_name + '_color.npz')
        file_index_train = os.path.join(save_root, train_set + '_index_train_ID' + str(nb_pt_each_batch) + '_pat' + str(nb_pat_each_pt) + '_split_' + split_name + '_color.npy')
    else:
    ...
    if os.path.exists(file_data_train):
        ...
        print(train_set)
        split_file = os.path.join(data_root, 'hputils/tasks/splits/splits.json')
...
def read_hpatches_patch_split_opencv(train_root, scene_train, sz_patch, color=True):
    sz_patch_raw = 65
    flag_resize = False
    if sz_patch_raw != sz_patch:
        flag_resize = True
...
def data_aug(patch, num_ID_per_batch, color=True):
    # sz = patch.size()
    patch.squeeze()
